...
public class BasicAttributeSensor<T> extends BasicSensor<T> implements AttributeSensor<T> {
    ...
    public BasicAttributeSensor(TypeToken<T> type, String name) {
        this((Class<T>) type.getRawType(), name);
    }

    @SuppressWarnings("unchecked")
    public BasicAttributeSensor(TypeToken<T> type, String name, String description) {
        this((Class<T>) type.getRawType(), name, description);
    }

    @SuppressWarnings("serial")
    public BasicAttributeSensor(String name) {
        this(new TypeToken<T>(BasicAttributeSensor.class) { }, name);
    }

    @SuppressWarnings("serial")
    public BasicAttributeSensor(String name, String description) {
        this(new TypeToken<T>(BasicAttributeSensor.class) { }, name, description);
    }
