...
public class ScheduledTask<T> extends BasicTask<T> {
    ...
    protected final Callable<Task<T>> taskFactory;
    ...
    protected Task<T> recentRun, nextRun;

    public int getRunCount() { return runCount; }

    public ScheduledFuture<T> getNextScheduled() { return (ScheduledFuture<T>) result; }

    public ScheduledTask(Callable<Task<T>> taskFactory) {
        this(MutableMap.of(), taskFactory);
    }

    public ScheduledTask(final Task<T> task){
        this(new Callable<Task<T>>(){
            @Override
    ...
    public ScheduledTask(Map<?, ?> flags, Callable<Task<T>> taskFactory) {
        super(flags);
    ...
    public ScheduledTask<T> delay(Duration d) {
        this.delay = d;
        return this;
    }

    public ScheduledTask<T> delay(long val) {
        return delay(Duration.millis(val));
    }

    public ScheduledTask<T> period(Duration d) {
        this.period = d;
        return this;
    }

    public ScheduledTask<T> period(long val) {
        return period(Duration.millis(val));
    }

    public ScheduledTask<T> maxIterations(int val) {
