...
public class BrooklynMementoPersisterToFile extends AbstractBrooklynMementoPersister {
    ...
    public BrooklynMemento loadMemento(LookupContext lookupContext) {
        ...
        synchronized (mutex) {
            ...
            try {
                BrooklynMemento result = readMemento();
                if (LOG.isDebugEnabled()) LOG.debug("Loaded memento; took {}",
    ...
    private BrooklynMemento readMemento() {
        try {
            return serializer.fromString(Files.asCharSource(file, Charsets.UTF_8).read());
        } catch (IOException e) {
