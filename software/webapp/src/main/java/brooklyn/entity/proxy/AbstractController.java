...
public interface AbstractController extends SoftwareProcess, LoadBalancer {
    ...
    BasicAttributeSensorAndConfigKey<AttributeSensor<Integer>> PORT_NUMBER_SENSOR = new BasicAttributeSensorAndConfigKey<AttributeSensor<Integer>>(
    ...
    PortAttributeSensorAndConfigKey PROXY_HTTP_PORT = new PortAttributeSensorAndConfigKey(
            "proxy.http.port", "Main HTTP port where this proxy listens", ImmutableList.of(8000, "8001+"));

    @SetFromFlag("protocol")
    BasicAttributeSensorAndConfigKey<String> PROTOCOL = new BasicAttributeSensorAndConfigKey<String>(
    ...
    @SetFromFlag("ssl")
    ConfigKey<ProxySslConfig> SSL_CONFIG = new BasicConfigKey<ProxySslConfig>(
    ...
    AttributeSensor<Set<String>> SERVER_POOL_TARGETS = new BasicAttributeSensor<Set<String>>(
