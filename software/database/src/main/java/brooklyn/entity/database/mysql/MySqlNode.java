...
public interface MySqlNode extends DatabaseNode, HasShortName {
    ...
    @SetFromFlag("version")
    ConfigKey<String> SUGGESTED_VERSION = ConfigKeys.newConfigKey(SoftwareProcess.SUGGESTED_VERSION, "5.5.30");
    ...
    ConfigKey<String> MIRROR_URL = ConfigKeys.newConfigKey("mysql.install.mirror.url", "URL of mirror",
...
//        "http://gd.tuwien.ac.at/db/mysql/"
    ...
    @SetFromFlag("port")
    PortAttributeSensorAndConfigKey MYSQL_PORT = ConfigKeys.newPortAttributeSensorAndConfigKey("mysql.port", "MySQL port", PortRanges.fromString("3306, 13306+"));

    @SetFromFlag("creationScriptContents")
    ConfigKey<String> CREATION_SCRIPT_CONTENTS = ConfigKeys.newConfigKey("mysql.creation.script.contents", "MySQL creation script (SQL contents)", "");

    @SetFromFlag("creationScriptUrl")
    ConfigKey<String> CREATION_SCRIPT_URL = ConfigKeys.newConfigKey("mysql.creation.script.url", "URL where MySQL creation script can be found", "");

    @SetFromFlag("dataDir")
    ConfigKey<String> DATA_DIR = ConfigKeys.newConfigKey("mysql.datadir", "Directory for writing data files", null);

    @SetFromFlag("serverConf")
    MapConfigKey<Object> MYSQL_SERVER_CONF = ConfigKeys.newMapConfigKey("mysql.server.conf", "Configuration options for mysqld");
    ...
    BasicAttributeSensorAndConfigKey<String> PASSWORD = ConfigKeys.newAttributeSensorAndConfigKey(
            "mysql.password", "Database admin password (or randomly generated if not set)", null);

    @SetFromFlag("socketUid")
    BasicAttributeSensorAndConfigKey<String> SOCKET_UID = ConfigKeys.newAttributeSensorAndConfigKey(
