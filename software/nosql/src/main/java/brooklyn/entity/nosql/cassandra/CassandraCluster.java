...
public interface CassandraCluster extends DynamicCluster {
    ...
    BasicAttributeSensorAndConfigKey<String> CLUSTER_NAME = new BasicAttributeSensorAndConfigKey<String>("cassandra.cluster.name", "Name of the Cassandra cluster", "BrooklynCluster");

    ConfigKey<String> SEEDS = new BasicConfigKey<String>("cassandra.cluster.seeds", "List of seed node hosts in cluster");

    AttributeSensor<String> HOSTNAME = new BasicAttributeSensor<String>("cassandra.cluster.hostname", "Hostname to connect to cluster with");

    AttributeSensor<Integer> THRIFT_PORT = new BasicAttributeSensor<Integer>("cassandra.cluster.thrift.port", "Cassandra Thrift RPC port to connect to cluster with");
