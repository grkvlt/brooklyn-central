...
public class VanillaJavaApp extends SoftwareProcessImpl implements UsesJava, UsesJmx, UsesJavaMXBeans {
    ...
    @SetFromFlag("args")
    public static final ConfigKey<List<?>> ARGS = ConfigKeys.newConfigKey("vanillaJavaApp.args", "Arguments for launching the java app", Lists.newArrayList());

    @SetFromFlag(value="main", nullable=false)
    public static final ConfigKey<String> MAIN_CLASS = ConfigKeys.newConfigKey("vanillaJavaApp.mainClass", "class to launch");

    @SetFromFlag("classpath")
    public static final ConfigKey<List<?>> CLASSPATH = ConfigKeys.newConfigKey("vanillaJavaApp.classpath", "classpath to use, as list of URL entries", Lists.newArrayList());

    @SetFromFlag
    protected long jmxPollPeriod;

    @SetFromFlag("jvmXArgs")
    public static final ConfigKey<List<String>> JVM_XARGS = ConfigKeys.newConfigKey("vanillaJavaApp.jvmXArgs", "JVM -X args for the java app (e.g. memory)",
        MutableList.of("-Xms128m", "-Xmx512m", "-XX:MaxPermSize=512m"));

    @SetFromFlag("jvmDefines")
    public static final MapConfigKey<Object> JVM_DEFINES = ConfigKeys.newMapConfigKey("vanillaJavaApp.jvmDefines", "JVM system property definitions for the app",
